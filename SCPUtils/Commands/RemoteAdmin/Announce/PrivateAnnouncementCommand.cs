namespace SCPUtils.Commands.RemoteAdmin.Announce
{
    ...
    public class PrivateAnnouncementCommand : ICommand
    {
        public string Command { get; } = "private";
        public string[] Aliases { get; } = new[] { "p", "player", "plr" };
        ...
        public bool Execute(ArraySegment<string> arguments, ICommandSender sender, out string response)
        {
            ...
            if (!sender.CheckPermission(ScpUtils.StaticInstance.perms.PermissionsList["scputils announce private"]))
            {
                response = ScpUtils.StaticInstance.commandTranslation.SenderError.Replace("%permission%", $"{ScpUtils.StaticInstance.perms.PermissionsList["scputils announce private"]}");
                return false;
            }
